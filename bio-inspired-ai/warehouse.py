...
class map():
    ...
    def gen(self):
        ...
        for n in range(0, len(self.obstacles)):
            if self.obstacles[n].start[0] == self.obstacles[n].end[0]:
                ...
                self.limv[n] = np.array([np.min([self.obstacles[n].start[1], self.obstacles[n].end[1]]) - 0.5,
            ...
            if self.obstacles[n].start[1] == self.obstacles[n].end[1]:
                ...
                self.limh[n] = np.array([np.min([self.obstacles[n].start[0], self.obstacles[n].end[0]]) - 0.5,
    ...
    def warehouse_map(self, width, height):
        box = make_box(height, width, [width / 2, height / 2]);
        [self.obstacles.append(box.walls[x]) for x in range(0, len(box.walls))]